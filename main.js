...
const startURL ='https://www.ford.com/dealerships/dealer-directory/browse-all/'
   // const google_access_token = await getGoogleToken(process.env.GOOGLE_REFRESH_TOKEN)
   const requestQueue = await Apify.openRequestQueue();
   requestQueue.addRequest({ url:startURL,userData:{pageType:'start'}  })
debugger;
    ...
    const handlePageFunction = async (context) => {
        ...
        if(pageType==='start'){
            ...
            const stateUrls = await page.evaluate(()=>Array.from(document.querySelectorAll('.columns li a')).map(a=> a.href))

            stateUrls.forEach(u=>{
                requestQueue.addRequest({ url:u,userData:{pageType:'state'}  })
            })

       
        }
        if(pageType==='state'){
    await page.waitForSelector('.dealer-item a')
    
            const dealerUrls = await page.evaluate(()=>Array.from(document.querySelectorAll('.dealer-item a')).map(u=> u.href))
            dealerUrls.forEach(u=>{
                requestQueue.addRequest({ url:u,userData:{pageType:'dealer info'}  })
            })
        ...
        if(pageType==='dealer info'){
            ...
            const {website,city,phone} =dealerInfo
            const google_access_token = await getGoogleToken(process.env.GOOGLE_REFRESH_TOKEN)
            debugger;
            const response= await appendSheetValues({ access_token: google_access_token, spreadsheetId: '1RuKUl0g275-FdlEiudy2-Po9F_7y-Pvt20pe4gbIJvM', range: 'DEALERS!A:B', values: [[website,city,phone]] })
            debugger;
        }
